...
class GPSTrainingGUI(object):
    ...
    def __init__(self, hyperparams):
        ...
        figure_title = self._hyperparams['experiment_dir'][self._hyperparams['experiment_dir'].index('/experiments/')
                                                      + len('/experiments/'):].split('/')[0]
        self._fig.canvas.set_window_title(figure_title)
        ...
        self._cost_plotter = MeanPlotter(self._fig, self._gs_cost_plotter,
                color='blue', label='mean cost', min_itr=self._hyperparams['iterations'])
    ...
    def update(self, itr, algorithm, agent, traj_sample_lists, pol_sample_lists, test=False, no_cost=False):
        """
        ...
        """
        if self._first_update and not test:
            self._output_column_titles(algorithm)
            self._first_update = False
        if not test and not no_cost:
        ...
        if END_EFFECTOR_POINTS in agent.x_data_types:
            self._update_trajectory_visualizations(algorithm, agent,
                    traj_sample_lists, pol_sample_lists, test=test)
    ...
    def _update_trajectory_visualizations(self, algorithm, agent,
                traj_sample_lists, pol_sample_lists, test=False):
        """
