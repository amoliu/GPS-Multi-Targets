...
def example_tf_network(is_training = True, dim_input=27, dim_output=7, batch_size=64, network_config=None, device_string="/cpu:0", weight_decay=0.005):
    """
    ...
    """
    dim_hidden = network_config.get('dim_hidden', [128, 64])
    dropout_keep_prob = network_config.get('dropout_keep_prob', 1.0)
    act_fn_name = network_config.get('activation_fn', 'relu')
    if act_fn_name == 'relu':
        act_fn = tf.nn.relu
        ...
        raise ValueError("Please select the activation function among these three choices: "
                         "\n (1) 'relu',  (2) 'tanh,  (3) 'elu' ")
...
def example_tf_network_with_BN(is_training, dim_input=27, dim_output=7, batch_size=64, network_config=None, device_string="/cpu:0", weight_decay=0.005):
    """
    ...
    with tf.device(device_string):
        ...
        with slim.arg_scope([slim.fully_connected],
            ...
            if dropout_keep_prob < 1.0:
                mlp_applied = slim.dropout(mlp_applied, keep_prob=dropout_keep_prob, scope='dropout')
            mlp_applied = slim.fully_connected(mlp_applied, num_outputs=dim_hidden[-1], activation_fn=None,
                                               scope='last_fc', normalizer_fn=None)
            mlp_applied = slim.fully_connected(mlp_applied, num_outputs=dim_output, scope='action_output',
