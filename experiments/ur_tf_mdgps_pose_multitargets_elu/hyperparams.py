...
def generate_goals(robot_chain, initial_bounds, target_bounds, initial_num, target_num, random=False):
    ...
    initial_state_cart = np.array([[-0.664, -0.164,  0.624]])
...
UR_PREFIXES = ['u0', 'u1', 'u2', 'u3', 'u4', 'u5', 'u6', 'u7', 'u8', 'u9']
...
target_state_num = [2, 5, 3] #Training conditions
# initial_state_num = [1, 1, 1]
# target_state_num = [3, 5, 4]
# boundaries for x, y, z coordinates
goal_position_bounds = np.array([[-0.80, -0.60],
                                 [-0.40, 0.40],
                                 [0.10, 0.50]])
...
angle_axis = np.array([[-np.pi / 2, 0, 1, 0],
                       [np.pi / 2, 1, 0, 0],
                       [0, 0, 0, 1]])
tgt_quaternion = []
EE_ROT_TGT = []
for idx in xrange(angle_axis.shape[0]):
    rotation_matrix = get_rotation_matrix(angle_axis[idx, 0], angle_axis[idx, 1:])
    tgt_quaternion.append(quaternion_from_matrix(rotation_matrix))
    EE_ROT_TGT.append(np.asmatrix(rotation_matrix[:3, :3]))
...
CONDITIONS = initial_joint_state.shape[0] * angle_axis.shape[0]
...
for i in xrange(common['conditions']):
    ...
    ee_pos_tgt = EE_POS_TGT[int(i / angle_axis.shape[0])]
    ee_rot_tgt = EE_ROT_TGT[int(i % angle_axis.shape[0])]
    ...
    trans, rot = forward_kinematics(ur_chain,
                                    m_link_names[0],
                                    INITIAL_JOINTS[int(i / angle_axis.shape[0])],
    ...
    reset_condition = {
        JOINT_ANGLES: INITIAL_JOINTS[int(i / angle_axis.shape[0])],
        JOINT_VELOCITIES: []
    }
    ...
    tgt_quaternions.append(tgt_quaternion[int(i % angle_axis.shape[0])])
