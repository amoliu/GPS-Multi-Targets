...
from gps.utility.general_utils import get_ee_points, get_position, inverse_kinematics
...
def generate_goals(robot_chain, initial_bounds, target_bounds, initial_num, target_num, random=False):
    ...
    # initial_joint_state = np.array([[0, -np.pi / 2, np.pi / 2, 0, 0, 0]])
...
# target_state_num = [3, 5, 4] #Training conditions
initial_state_num = [1, 1, 1]
target_state_num = [3, 5, 4]
# boundaries for x, y, z coordinates
goal_position_bounds = np.array([[-0.8, -0.40],
                                 [-0.6, 0.6],
                                 [-0.45, 0.45]])
...
# EE_POS_TGT = np.asmatrix([[-0.70, -0.70, 0.50],
#                           [-0.60, -0.70, 0.50],
#                           [-0.60, -0.60, 0.50],
#                           [-0.70, -0.60, 0.50]])
...
UR_GAINS = np.array([2.195, 0.922, 0.582, 0.393, 0.151, 0.152])
...
EXP_DIR = os.path.dirname(os.path.realpath(__file__)) + '/test_sparse/'
...
for i in xrange(common['conditions']):
    ...
    #
    # x0[joint_dim:(joint_dim + EE_POINTS.size)] = get_position(tf, LINK_NAMES[-1], LINK_NAMES[0], time)
    ...
    x0[joint_dim:(joint_dim + EE_POINTS.size)] = trans
...
action_cost = {
    'type': CostAction,
    'wu': 0.022 / UR_GAINS #0.02 * np.ones(SENSOR_DIMS[ACTION]) #good for slowness = 1.0
